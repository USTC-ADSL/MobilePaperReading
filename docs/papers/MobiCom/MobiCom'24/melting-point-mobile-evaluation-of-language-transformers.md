...
文章的测量指标侧重于用户日常体验的角度，其中持续性能稳定性是一个很有启发性的测量指标，通过50轮长对话测试，检测该推理过程种的各性能指标来分析各端侧后端在运行推理任务时的稳定性。此外，文章自主搭建了一个全流程自动化的测试平台，使得整个实验的工作流清晰明了，便于后续对数据的解释与分析，启示我们今后在做测试相关的工作时应当注重测试流程的完备性，否则测试流程本身会对实验数据产生主观上容易忽略的影响。
